...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "before deciding next steps. Do NOT blindly retry; verify first.\n"
        "- `effect: 'suspected_noop'` or `code: 'background_unavailable'` — "
        "the action did NOT land. Follow `escalation.recommended` if present:\n"
        ...
        "- An `escalation.recommended` field attached to `unverifiable` is "
        "a fallback suggestion IF verification shows the action did not land "
        "— it is NOT proof that the action failed on its own. Always verify "
        "with a fresh capture before deciding to escalate.\n"
